  ...
  //모달에서 변경 누르면 input 데이터 저장. Nick.js에서 데이터를 부르고 rest API로 data.json으로 전달(변경)
    return (
      <div >
        ...
        <React.Fragment>
              <button className="changeNick" onClick={ openModal }>닉네임 변경</button>
              <NickModal open={ modalOpen } change={ changeModal }
             close={ closeModal } header="닉네임 변경"
              >
               <div className="nickname">
                ...
                <div className="value">
                <input type="text" onChange={onChange} value={text} name="nickname"/>
                </div>               </NickModal>
          </React.Fragment>
      </div>
    );
